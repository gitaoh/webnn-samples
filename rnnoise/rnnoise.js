...
export class RNNoise {
	...
	async load(contextOptions) {
		...
		const biasedTensorName2 = this.builder_.add(
			inputDense0,
			inputDenseBias0,
		);
		const inputDenseTanh0 = this.builder_.tanh(biasedTensorName2);
		const vadGruX = this.builder_.transpose(inputDenseTanh0, {
			permutation: [1, 0, 2],
		});
		...
		this.vadGruInitialHTensor_ =
			await this.context_.createTensor(vadGruInitialHDesc);
		...
		const vadGruTranspose1 = this.builder_.reshape(vadGruYTransposed, [
			1,
			this.frames_,
			this.vadGruHiddenSize,
		]);
		const concatenate1 = this.builder_.concat(
			[inputDenseTanh0, vadGruTranspose1, input],
			2,
		);
		const noiseGruX = this.builder_.transpose(concatenate1, {
			permutation: [1, 0, 2],
		});
		...
		this.noiseGruInitialHTensor_ =
			await this.context_.createTensor(noiseGruInitialHDesc);
		...
		const noiseGruTranspose1 = this.builder_.reshape(noiseGruYTransposed, [
			1,
			this.frames_,
			this.noiseGruHiddenSize,
		]);
		const concatenate2 = this.builder_.concat(
			[vadGruTranspose1, noiseGruTranspose1, input],
			2,
		);
		const denoiseGruX = this.builder_.transpose(concatenate2, {
			permutation: [1, 0, 2],
		});
		...
		const denoiseGruTranspose1 = this.builder_.reshape(
			denoiseGruYTransposed,
			[1, this.frames_, this.denoiseGruHiddenSize],
		);
		const denoiseOutput0 = this.builder_.matmul(
			denoiseGruTranspose1,
			denoiseOutputKernel0,
		);
	...
	async compute(inputs) {
		...
		this.context_.writeTensor(
			this.vadGruInitialHTensor_,
			inputs.vadGruInitialH,
		);
		this.context_.writeTensor(
			this.noiseGruInitialHTensor_,
			inputs.noiseGruInitialH,
		);
		this.context_.writeTensor(
			this.denoiseGruInitialHTensor_,
			inputs.denoiseGruInitialH,
		);
		const inputTensors = {

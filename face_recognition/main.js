...
async function predict(targetElem, searchElem) {
	...
	if (flag1 && flag2) {
		computeTime =
			targetEmbeddings.computeTime + searchEmbeddings.computeTime;
	} else if (flag1 && !flag2) {
...
async function main() {
	try {
		...
		if (inputType === "image") {
			...
			console.log("- Computing... ");
			// Do warm up
			await fdInstance.compute(
				new Float32Array(utils.sizeOfShape(fdInputOptions.inputShape)),
			);
			await frInstance.compute(
				new Float32Array(utils.sizeOfShape(frInputOptions.inputShape)),
			);

...
export function decodeOutputBoxTensor(options, outputBoxTensor, anchors) {
	const { boxSize = 4, numBoxes = 1083 + 600 + 150 + 54 + 24 + 6 } = options;
	...
	for (let x = 1; x < numClasses; ++x) {
		...
		for (let y = 0; y < numBoxes; ++y) {
			...
			if (outputClassScoresTensor[scoreIndex] > scoreThreshold) {
				...
				boxes.push(
					outputBoxTensor.subarray(
						boxIndexStart,
						boxIndexStart + boxSize,
					),
				);
...
export function cropSsdBox(imageSource, totalDetections, boxesList, margin) {
	...
	for (let i = 0; i < totalDetections; ++i) {
		...
		boxesList[i][0] = Math.max(
			0,
			(ymax + ymin) / 2 - ((ymax - ymin) / 2) * margin[2],
		);
		boxesList[i][2] = Math.min(
			imHeight,
			(ymax + ymin) / 2 + ((ymax - ymin) / 2) * margin[3],
		);
		boxesList[i][1] = Math.max(
			0,
			(xmax + xmin) / 2 - ((xmax - xmin) / 2) * margin[0],
		);
		boxesList[i][3] = Math.min(
			imWidth,
			(xmax + xmin) / 2 + ((xmax - xmin) / 2) * margin[1],
		);
	}
...
export function processSsdOutputTensor(
	outputSsdTensor,
	inputOptions,
	outputsInfo,
) {
	...
	for (let i = 0; i < numBoxes.length; ++i) {
		...
		if (inputOptions.inputLayout === "nchw") {
			boxTensor = tf.tidy(() => {
				const a = tf.tensor(
					outputSsdTensor[2 * i],
					Object.entries(outputsInfo)[2 * i][1],
					"float32",
				);
			...
			classTensor = tf.tidy(() => {
				const a = tf.tensor(
					outputSsdTensor[2 * i + 1],
					Object.entries(outputsInfo)[2 * i + 1][1],
					"float32",
				);
				const b = tf.transpose(a, [0, 2, 3, 1]);
				return b.dataSync();

...
numpy.Array = class {
	constructor(buffer) {
		if (buffer) {
			...
			if (major !== 1 && minor !== 0) {
				throw new numpy.Error(
					"Invalid version '" + [major, minor].join(".") + "'.",
				);
			}
			...
			switch (this._byteOrder) {
				...
				case "<": {
					if (header.descr.length !== 3) {
						throw new numpy.Error(
							"Unsupported data type '" + header.descr + "'.",
						);
					}
					this._dataType = header.descr.substring(1);
					const size =
						parseInt(header.descr[2], 10) *
	...
	toBuffer() {
		...
		if (context.byteOrder !== "<" && context.byteOrder !== ">") {
			throw new numpy.Error(
				"Unknown byte order '" + this._byteOrder + "'.",
			);
		}
		if (
			context.dataType.length !== 2 ||
			(context.dataType[0] !== "f" &&
				context.dataType[0] !== "i" &&
				context.dataType[0] !== "u")
		) {
			throw new numpy.Error(
				"Unsupported data type '" + this._dataType + "'.",
			);
		}
		...
		switch (this._shape.length) {
			...
			default:
				shape =
					"(" +
					this._shape
						.map((dimension) => dimension.toString())
						.join(", ") +
					")";
				break;
		}

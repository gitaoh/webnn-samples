...
export class NNotepad {
					...
					.map((k) => {
						...
						const argumentType =
							typeof callContext === "object" ?
								...
								kArgTypeNonOperand;
						return `${Util.stringify(k)}: ${serializeExpr(
							v,
							argumentType,
						)}`;
					})
	...
	static addMonacoLanguage(monaco) {
		...
		monaco.languages.setLanguageConfiguration(
			...
			NNotepad.monacoLanguageConfiguration,
		);
			...
			monaco.languages.registerCompletionItemProvider(
				NNotepad.monacoLanguageId,
				{
					provideCompletionItems: (model, position) => {
						const suggestions = methods.map((name) => ({
							label: name,

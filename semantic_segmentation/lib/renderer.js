...
export class Renderer {
	constructor(canvas) {
		...
		const kernel1D = this.generateGaussianKernel1D_(
			this.blurRadius_ * 2 + 1,
		);
		...
		this.colorPalette_ = new Uint8Array([
			45, 52, 54, 85, 239, 196, 129, 236, 236, 116, 185, 255, 162, 155,
			254, 223, 230, 233, 0, 184, 148, 0, 206, 201, 9, 132, 227, 39, 60,
			117, 108, 92, 231, 178, 190, 195, 255, 234, 167, 250, 177, 160, 255,
			118, 117, 253, 121, 168, 99, 110, 114, 253, 203, 110, 225, 112, 85,
			214, 48, 49, 232, 67, 147,
		]);
	}
	...
	setupExtractShader_() {
		...
		const vsWithoutPreprocess = `#version 300 es
      in vec4 a_pos;
      out vec2 v_texcoord;
      out vec2 v_maskcord;
      ...
      void main() {
		...
		const vsWithPreprocess = `#version 300 es
      in vec4 a_pos;
      out vec2 v_texcoord;
      out vec2 v_maskcord;
	...
	async uploadNewTexture(imageSource, clippedSize) {
		if (
			this.clippedSize_[0] !== clippedSize[0] ||
	...
	drawColorLabel_() {
		...
		for (const labelId of uniqueLabels) {
			...
			const rgbTuple = this.colorPalette_.slice(
				labelId * 3,
				(labelId + 1) * 3,
			);
		...
		for (const id in labelMap) {
			if (!shownLabelId.has(id)) {
				const labelDiv = $(
					...
					.append(
						$(
							`<span style="color:rgb(${labelMap[id][1]})">⬤</span>`,
						),
					)

...
async function executeCodeSnippet(consoleLogElement, codeSnippet) {
	...
	function reportError(e) {
		...
		errorMessage += "<br/>";
		errorMessage +=
			'<div class="snippet-error-msg">' + e.message + "</div>";
		errorMessage += "</div>";
...
function initCodeBlocks(selector) {
	...
	var jsBlocks = Array.prototype.slice.call(
	...
	jsBlocks.forEach(function (block) {
		...
		consoleRunElement.addEventListener("click", function () {
			var consoleLogElement = this.parentElement.querySelector(
				".snippet-console-log",
			);

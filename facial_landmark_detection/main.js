...
async function predict(inputElement) {
	...
	for (let i = 0; i < totalDetections; ++i) {
		...
		const fldInputBuffer = utils.getInputTensor(
			inputElement,
			fldInputOptions,
		);
	...
	try {
		...
		if (
			...
			if (lastdeviceType != deviceType || lastBackend != backend) {
				...
				lastdeviceType =
					lastdeviceType != deviceType ? deviceType : lastdeviceType;
		...
		if (inputType === 'image') {
			...
			const fldResults = await fldInstance.compute(
				new Float32Array(utils.sizeOfShape(fldInputOptions.inputShape)),
			);
			for (let i = 0; i < numRuns; i++) {
				[computeTime, strokedRects, keyPoints] =

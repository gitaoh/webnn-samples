...
export class EfficientNetFP16Nchw {
	...
	async buildConv_(input, name, blockName, clip = false, options = {}) {
		let prefix = '';
		if (blockName !== '') {
			prefix = this.weightsUrl_ + 'block' + blockName + '_conv' + name;
		} else {
	...
	async buildGemm_(input, name) {
		...
		const weight = buildConstantByNpy(this.builder_, weightName, 'float16');
		const biasName = prefix + '_b.npy';
		const bias = buildConstantByNpy(this.builder_, biasName, 'float16');
		const options = {c: this.builder_.reshape(await bias, [1, 1000])};
	...
	async buildBottlenecks_(input, blockNames, group, pad = 1) {
		...
		for (let i = 0; i < blockNames.length; i++) {
			const bottleneck = await this.buildBottleneck_(
				result,
				blockNames[i],
				group,
				pad,
	...
	async load(contextOptions) {
		...
		// Block 0
		const conv1 = this.buildConv_(data, '0', '0', true, {
			padding: [0, 1, 0, 1],
			strides: [2, 2],
		});
		...
		// Block 2~4
		const bottleneck4 = this.buildBottlenecks_(conv6, ['2', '3', '4'], 192);
		...
		// Block 6~8
		const bottleneck8 = this.buildBottlenecks_(
			conv9,
			['6', '7', '8'],
			336,
			2,
		...
		// Block 10~14
		const bottleneck14 = this.buildBottlenecks_(
			conv12,
			['10', '11', '12', '13', '14'],
			672,
		);
		...
		// Block 16~20
		const bottleneck20 = await this.buildBottlenecks_(
			conv15,
			['16', '17', '18', '19', '20'],
			960,
			2,
		...
		// Block 22~28
		const bottleneck28 = this.buildBottlenecks_(
			conv18,

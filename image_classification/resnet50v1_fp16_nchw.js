...
export class ResNet50V1FP16Nchw {
	...
	async buildConv_(input, name, stageName, relu, options = undefined) {
		let prefix = "";
		if (stageName !== "") {
			prefix = this.weightsUrl_ + "stage" + stageName + "_conv" + name;
		} else {
	...
	async buildBottleneck_(
		...
		const conv1 = this.buildConv_(input, nameIndex, stageName, true, {
			strides,
		});
		const conv2 = this.buildConv_(
			conv1,
			parseInt(nameIndex) + 1,
			stageName,
			true,
			{ padding: [1, 1, 1, 1] },
		);
	...
	async load(contextOptions) {
		...
		const conv1 = await this.buildConv_(data, "0", "", true, {
			padding: [3, 3, 3, 3],
			strides: [2, 2],
		});
		...
		// Stage 2
		const bottleneck4 = this.buildBottleneck_(
			bottleneck3,
			"2",
			"0",
			true,
		...
		// Stage 3
		const bottleneck8 = this.buildBottleneck_(
			bottleneck7,
			"3",
			"0",
			true,
		...
		// Stage 4
		const bottleneck14 = this.buildBottleneck_(
			bottleneck13,
			"4",
			"0",
			true,

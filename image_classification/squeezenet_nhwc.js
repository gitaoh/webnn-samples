...
export class SqueezeNetNhwc {
	constructor() {
		...
		this.weightsUrl_ =
			weightsOrigin() + "/test-data/models/squeezenet1.0_nhwc/weights/";
		this.inputOptions = {
	...
	async buildFire_(input, name) {
		...
		const convE3x3 = this.buildConv_(convSqueeze, name + "_e3x3", {
			padding: [1, 1, 1, 1],
		});
	...
	async load(contextOptions) {
		...
		const conv1 = this.buildConv_(placeholder, "conv1", {
			strides,
			autoPad: "same-upper",
		});
		const maxpool1 = this.builder_.maxPool2d(await conv1, {
		...
		const maxpool4 = this.builder_.maxPool2d(await fire4, {
			windowDimensions: [3, 3],
			strides,
			layout,
		});
		...
		const maxpool8 = this.builder_.maxPool2d(await fire8, {
			windowDimensions: [3, 3],
			strides,
			layout,
		});
		...
		const averagePool2d = this.builder_.averagePool2d(await conv10, {

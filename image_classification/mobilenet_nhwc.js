...
export class MobileNetV2Nhwc {
	constructor() {
		...
		this.weightsUrl_ =
			weightsOrigin() + '/test-data/models/mobilenetv2_nhwc/weights/';
		this.inputOptions = {
	...
	async buildConv_(input, weightsSubName, biasSubName, relu6, options) {
		const weightsName =
			this.weightsUrl_ + 'Const_' + weightsSubName + '.npy';
		const weights = await buildConstantByNpy(this.builder_, weightsName);
		const biasName =
			this.weightsUrl_ + 'MobilenetV2_' + biasSubName + '_bias.npy';
	...
	async load(contextOptions) {
		...
		const conv0 = this.buildConv_(input, '90', 'Conv_Conv2D', true, {
			strides,
			autoPad,
			filterLayout,
		});
		const conv1 = this.buildConv_(
			await conv0,
			'238',
			'expanded_conv_depthwise_depthwise',
			true,
		...
		const averagePool2d = this.builder_.averagePool2d(await conv3, {

...
export class ResNet50V2Nhwc {
	constructor() {
		...
		this.weightsUrl_ =
			weightsOrigin() + "/test-data/models/resnet50v2_nhwc/weights/";
		this.inputOptions = {
	...
	async buildConv_(input, nameIndices, options = {}, relu = true) {
		...
		if (options.autoPad == "same-upper") {
			...
			const inputShape = isShapeMethod ?
				(await input).shape() :
				(await input).shape;
			const weightsShape = isShapeMethod ?
				weights.shape() :
				weights.shape;
	...
	async buildBottleneckV2_(
		...
		if (downsample) {
			residual = this.buildConv_(
				await fusedBn,
				nameIndices.concat(["shortcut"]),
				{ autoPad },
				false,
			);
		}
		if (!downsample && shortcut) {
			residual = this.builder_.maxPool2d(await input, {
				windowDimensions: [2, 2],
				strides,
				layout,
				autoPad,
			});
			conv2 = this.buildConv_(await conv1, nameIndices.concat(["2"]), {
				strides,
				padding: [1, 1, 1, 1],
			});
		} else {
	...
	async load(contextOptions) {
		...
		const conv1 = await this.buildConv_(
			input,
			["", "", "1"],
			{ strides, padding: [3, 3, 3, 3] },
			false,
		);
		const windowDimensions = [3, 3];
		const conv1Shape =
			typeof conv1.shape === "function" ? conv1.shape() : conv1.shape;
		const pool = this.builder_.maxPool2d(conv1, {
			windowDimensions,
			strides,
			layout,
		...
		const bottleneck3 = this.buildBottleneckV2_(bottleneck2, ["1", "3"]);
		...
		const bottleneck7 = this.buildBottleneckV2_(bottleneck6, ["2", "4"]);
		...
		const bottleneck10 = this.buildBottleneckV2_(bottleneck9, ["3", "6"]);
		...
		const fusedBn = this.buildFusedBatchNorm_(bottleneck13, ["postnorm"]);

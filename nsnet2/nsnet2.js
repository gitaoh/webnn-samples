...
export class NSNet2 {
	...
	async load(contextOptions, baseUrl, batchSize, frames) {
		...
		const relu20 = this.builder_.relu(
			this.builder_.add(
				this.builder_.matmul(input, weight172),
				biasFcIn0,
			),
		);
		...
		this.initialState92Tensor_ =
			await this.context_.createTensor(initialStateDesc);
		this.initialState155Tensor_ =
			await this.context_.createTensor(initialStateDesc);
		...
		const squeeze158Shape = isShapeMethod ?
			gru156.shape() :
			[...gru156.shape];
		...
		const relu167 = this.builder_.relu(
			this.builder_.add(
				this.builder_.matmul(relu163, weight216),
				biasFcOut2,
			),
		);
		const output = this.builder_.sigmoid(
			this.builder_.add(
				this.builder_.matmul(relu167, weight217),
				biasFcOut4,
			),
		);
	...
	async compute(inputBuffer, initialState92Buffer, initialState155Buffer) {
		...
		const results = {
			output: new Float32Array(
				await this.context_.readTensor(this.outputTensor_),
			),
			gru94: new Float32Array(
				await this.context_.readTensor(this.gru94Tensor_),
			),
			gru157: new Float32Array(
				await this.context_.readTensor(this.gru157Tensor_),
			),
		};
		return results;
	}

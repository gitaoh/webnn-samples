...
export class Denoiser {
	...
	async process(audioData, callback) {
		...
		let initialHiddenState92Buffer = new Float32Array(
			1 * this.batchSize * this.nsnet.hiddenSize,
		);
		let initialHiddenState155Buffer = new Float32Array(
			1 * this.batchSize * this.nsnet.hiddenSize,
		);
		...
		for (
			...
			const outputs = await this.nsnet.compute(
				inputData,
				initialHiddenState92Buffer,
				initialHiddenState155Buffer,
			);
			...
			const sigOut = tf.tidy(() => {
				...
				if (frame === 0) {
					...
					if (lastIteration) {
						...
						sliceSize =
							sigOut.shape[0] -
							overlap * sizePerFrame -
							sizePerFrame;
					}
				} else if (lastIteration) {
					sliceStart = overlap * sizePerFrame;
					sliceSize =
						sigOut.shape[0] - endPadding - overlap * sizePerFrame;
				} else {
					sliceStart = overlap * sizePerFrame;
					sliceSize =
						sigOut.shape[0] -
						2 * overlap * sizePerFrame -
						sizePerFrame;
				}

...
onmessage = async (message) => {
	if (message) {
		...
		case 'load': {
			...
			modelRunnerResult =
					module.TFLiteWebModelRunner.CreateFromBufferAndOptions(
						...
						modelBytes.length,
						{
							numThreads: 1,
							enableWebNNDelegate:
								message.data.enableWebNNDelegate,
							webNNDevicePreference: parseInt(
								message.data.webNNDevicePreference,
							),
							webNNNumThreads: parseInt(
								message.data.webNNNumThreads,
							),
						},
					);
			...
			const loadFinishedMs = (performance.now() - loadStart).toFixed(
				2,
			);
		...
		case 'compute': {
			...
			const inputs = callAndDelete(
				modelRunner.GetInputs(),
				(results) => convertCppVectorToArray(results),
			);
			const input = inputs[0];
			const outputs = callAndDelete(
				modelRunner.GetOutputs(),
				(results) => convertCppVectorToArray(results),
			);

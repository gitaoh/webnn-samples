...
export class LeNet {
	...
	async load(contextOptions) {
		...
		const add1BiasData = new Float32Array(
			arrayBuffer,
			byteOffset,
			sizeOfShape(add1BiasShape),
		);
		...
		byteOffset +=
			sizeOfShape(add1BiasShape) * Float32Array.BYTES_PER_ELEMENT;
		...
		const pool1 = this.builder_.maxPool2d(conv1, {
			windowDimensions: pool1WindowShape,
		...
		const add2Bias = this.builder_.constant(
			...
			new Float32Array(
				arrayBuffer,
				byteOffset,
				sizeOfShape(add2BiasShape),
			),
		);
		byteOffset +=
			sizeOfShape(add2BiasShape) * Float32Array.BYTES_PER_ELEMENT;
		...
		let pool2 = this.builder_.maxPool2d(conv2, {
			windowDimensions: pool2WindowShape,
		...
		const matmul1Weights = this.builder_.constant(
			...
			new Float32Array(
				arrayBuffer,
				byteOffset,
				sizeOfShape(matmul1Shape),
			),
		);
		byteOffset +=
			sizeOfShape(matmul1Shape) * Float32Array.BYTES_PER_ELEMENT;
		const matmul1WeightsTransposed =
			this.builder_.transpose(matmul1Weights);
		...
		const add3Bias = this.builder_.constant(
			...
			new Float32Array(
				arrayBuffer,
				byteOffset,
				sizeOfShape(add3BiasShape),
			),
		);
		byteOffset +=
			sizeOfShape(add3BiasShape) * Float32Array.BYTES_PER_ELEMENT;
		...
		const matmul2Weights = this.builder_.constant(
			...
			new Float32Array(
				arrayBuffer,
				byteOffset,
				sizeOfShape(matmul2Shape),
			),
		);
		byteOffset +=
			sizeOfShape(matmul2Shape) * Float32Array.BYTES_PER_ELEMENT;
		const matmul2WeightsTransposed =
			this.builder_.transpose(matmul2Weights);
